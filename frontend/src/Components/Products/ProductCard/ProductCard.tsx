...
export const ProductCard: React.FC<PropsType> = ({ product, setNewOrder }) => {
	const [image, setImage] = React.useState(
		`https://YOUR-SERVER-API-URI/${product.firstType.src}`
	)
	...
	const onMouseOverHandler = (imageSrc: string, productID: string) => {
		setImage(`https://YOUR-SERVER-API-URI/${imageSrc}`)
	...
	return (
		<div className={style.cardWrapper}>
			<ul className={style.thumb}>
				<li
					onMouseOver={() =>
						onMouseOverHandler(product.firstType.src, product.firstType.id)
					}
				>
					<img
						src={`https://YOUR-SERVER-API-URI/${product.firstType.src}`}
						alt='product'
					/>
				</li>
				<li
					onMouseOver={() =>
						onMouseOverHandler(product.secondType.src, product.secondType.id)
					}
				>
					<img
						src={`https://YOUR-SERVER-API-URI/${product.secondType.src}`}
						alt='product'
					/>
				</li>
				<li
					onMouseOver={() =>
						onMouseOverHandler(product.thirdType.src, product.thirdType.id)
					}
				>
					<img
						src={`https://YOUR-SERVER-API-URI/${product.thirdType.src}`}
						alt='product'
					/>
				</li>
			</ul>
